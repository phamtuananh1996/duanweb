...
class ListCategoryComposer
{
	public function compose(View $view){
        ...
        foreach ($category as $key => $value) {
            if($value->super_category_id == 0){
                echo '<tr>';
                echo '<td><span class="white-text" style="margin-left: 2em;"><strong>'.$value->title.'</strong></span></td>';
                echo '<td>';
                    ...
                    echo '</div>';
                echo '</td>';
                $this->sublist($category,$super_category_id=$value->id);
                echo '</tr>';
            }
        }
        echo '</tbody>';
	...
	function sublist($category, $super_category_id)
    {
        $cate_data = array();
        ...
        if ($cate_data)
        {
            ...
            {
                echo '<tr>';
                echo '<td><span class="white-text" style="margin-left: 5em;">'.$item->title.'</span></td>';
                echo '<td>';
                    echo '<div class="btn-group">';
                    ...
                    echo '</div>';
                echo '</td>';
                $this->sublist($category, $item['super_category_id']);
                echo '</tr>';
            }
        }
    }
